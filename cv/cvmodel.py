...
def process_img(image_path, model):
    ...
    horizontal_kernel = np.ones((10,10),np.uint8)#cv2.getStructuringElement(cv2.MORPH_RECT, (25,1))
    ...
    for c in cnts:
        ...
        if w<20 or h<20:
            continue
        # remove monkey piece of the hands
        if (53<w<59) and (44<h<50):
            continue
        # case where two boxes are stacked on top of each other
        if 46 <= w <= 50 and 96 <= h <= 100: 
            h_half = h // 2
            cnts_process.append((x, y, w, h_half))
            cnts_process.append((x, y + h_half, w, h_half))
            continue  # Skip adding the full ROI
        # case where two boxes are stacked horizontally
        if 96 <= w <= 100 and 46 <= h <= 50:
            w_half = w // 2
            cnts_process.append((x, y, w_half, h))
            cnts_process.append((x + w_half, y, w_half, h))
            continue
        # case where two bananas are next to each other
        if 33 <= h <= 37 and 98 <= w <= 102:
            w_half = w // 2
            cnts_process.append((x, y, w_half, h))
            cnts_process.append((x + w_half, y, w_half, h))
            continue
        cnts_process.append((x, y, w, h))

    for (x, y, w, h) in cnts_process:
    ...
    height, width = image.shape[:2]    
    # Create a copy of just the top half
    top_half = image[0:height//2, 0:width].copy()
    # Return top half instead of full image
    image = top_half
...
if __name__ == "__main__":
    model = load_model()
    img_path = '..\\Images for Demo\\E.png'
    label_encoder = joblib.load('label_encoder.pkl')
    image, info, rois, imgs = process_img(img_path, model)
